...
// Column mapping taken from the ai-chatbot template's lib/db/schema.ts: `Chat` holds (id,
// createdAt, title, userId, visibility) and `Message_v2` holds (id, chatId, role, parts,
// attachments, createdAt). Its parts are already UIMessage parts, which is why an import is a copy.
 ...
 * list ordered by time, so they are linked into the parent chain this schema expects.
 */
