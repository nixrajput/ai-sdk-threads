...
// ponytail: prefix match, not a manifest - fine while ours are the two known entries.
const canonical = (name) => name.replace(/-[A-Za-z0-9_-]{8,}(?=\.)/, "-*");

/** Recurses so subpath entries (dist/drizzle/index.js) are measured, not just the root. */
function measureDist(dir, prefix = "") {
  return readdirSync(dir, { withFileTypes: true })
    .flatMap((entry) => {
      if (entry.isDirectory()) {
        return measureDist(join(dir, entry.name), `${prefix}${entry.name}/`);
      }
      if (!entry.name.endsWith(".js") && !entry.name.endsWith(".d.ts")) return [];
      const buf = readFileSync(join(dir, entry.name));
      return [
        { name: canonical(`${prefix}${entry.name}`), raw: buf.length, gz: gzipSync(buf).length },
      ];
    })
...
function publishedBaseline() {
  const dir = mkdtempSync(join(tmpdir(), "ai-sdk-threads-baseline-"));
  try {
...
function bench() {
  const out = run("npx", ["vitest", "bench", "--run"], {
    AI_SDK_THREADS_BENCH_TIME_MS: FAST ? "100" : "500",
  });
  const table = out
    .split("\n")
...
function localMinified() {
  const dir = mkdtempSync(join(tmpdir(), "ai-sdk-threads-min-"));
  try {
    run("npx", [
      "tsdown",
      "src/index.ts",
